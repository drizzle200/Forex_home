...
class FundedAccount(models.Model):
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='funded_accounts')
    ...
    def __str__(self):
        return f"{self.account_name} - ${self.account_balance} ({self.account_type})"
...
class Trades(models.Model):
    ...
    account = models.ForeignKey(FundedAccount, on_delete=models.SET_NULL, null=True, blank=True, related_name='trades')
    risk_percent = models.DecimalField(max_digits=5, decimal_places=2, null=True, blank=True)
    stop_loss_pips = models.IntegerField(null=True, blank=True)
    calculated_lot_size = models.DecimalField(max_digits=10, decimal_places=3, null=True, blank=True)
    pip_value_used = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True)


    pair   = models.ForeignKey(Pairs, on_delete=models.CASCADE, related_name="trades")
    momentum_h4   = models.CharField(max_length=100, null=True,blank=True, choices=H4_CHOICES)
    momentum_h1   = models.CharField(max_length=100, null=True, blank=True, choices=H4_CHOICES)
    momentum_15m  = models.CharField(max_length=100, null=True,blank=True,  choices=H4_CHOICES)
    momentum_5m   = models.CharField(max_length=100, null=True, blank=True, choices=H4_CHOICES)
    momentum_1m   = models.CharField(max_length=100, null=True,blank=True,  choices=H4_CHOICES)
    session = models.CharField(max_length=100, null=True,blank=True)
    entry_place   = models.CharField(max_length=100, null=True,blank=True,  choices=ENTRY_PLACE_CHOICES)
    buy_or_sell   = models.CharField(max_length=100, null=True,blank=True, choices=BUY_CHOICES)
    ...
    confirmation   = models.CharField(max_length=100, null=True, blank=True, choices=CONFIRMATION_CHOICES)
    mood		   = models.CharField(max_length=100, null=True,blank=True, choices=MOOD_CHOICES)
    tp  		 = models.CharField(max_length=100, null=True, blank=True, choices= TP_CHOICES)
    ...
    rvs_grade   = models.CharField(max_length=100,blank=True,  null=True)
    target 		   = models.IntegerField(null=True, blank=True, choices=TARGET_CHOICES)
    reason 		   = models.CharField(max_length=100, null=True,blank=True,  choices=REASON_CHOICES)
    ...
    is_draft = models.BooleanField(
        default=False,
        help_text="True if this trade is saved as a draft"
    )
...
class GlobalTrainingData(models.Model):
    """
    Global dataset used for training the prediction model.
    This data is NOT user-specific and is shared across all users.
    """
    # Same structure as Trades but without user FK
    pair = models.ForeignKey(Pairs, on_delete=models.CASCADE)
    momentum_h4 = models.CharField(max_length=100, null=True, blank=True)
    momentum_h1 = models.CharField(max_length=100, null=True, blank=True)
    momentum_15m = models.CharField(max_length=100, null=True, blank=True)
    momentum_5m = models.CharField(max_length=100, null=True, blank=True)
    momentum_1m = models.CharField(max_length=100, null=True, blank=True)
    session = models.CharField(max_length=100, null=True, blank=True)
    entry_place = models.CharField(max_length=100, null=True, blank=True)
    buy_or_sell = models.CharField(max_length=100, null=True, blank=True)
    setup_quality = models.IntegerField(null=True, blank=True)
    ...
    tp = models.CharField(max_length=100, null=True, blank=True)
    tp_reason = models.CharField(max_length=50, null=True, blank=True)
    risk_reward = models.FloatField(null=True, blank=True)
    target = models.IntegerField(null=True, blank=True)  # 0=loss, 1=win
    
    # Metadata
    SOURCE_CHOICES = [
        ('initial_seed', 'Initial Seed'),
        ('aggregated', 'Aggregated from Users'),
        ('manual', 'Manually Added'),
    ]
    source = models.CharField(max_length=50, default='initial_seed', choices=SOURCE_CHOICES)
    created_at = models.DateTimeField(auto_now_add=True)
    is_active = models.BooleanField(default=True)
    ...
    def __str__(self):
        return f"Global Trade {self.id} - {'Win' if self.target == 1 else 'Loss' if self.target == 0 else 'Unknown'}"
