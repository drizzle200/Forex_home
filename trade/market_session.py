...
def get_trading_session():
    ...
    try:
        ...
        # WEEKEND CHECK - Forex opens Sunday 5:00 PM EST, closes Friday 5:00 PM EST
        weekday_est = now_est.weekday()  # 0 = Monday, 4 = Friday, 5 = Saturday, 6 = Sunday
        
        print(f"DEBUG - Current EST: {now_est.strftime('%Y-%m-%d %H:%M:%S %Z')}")
        print(f"DEBUG - Weekday EST: {weekday_est} (0=Mon, 4=Fri, 5=Sat, 6=Sun)")
        print(f"DEBUG - Hour EST: {hour_est}, Minute: {minute_est}")
        
        # Check if market is closed for weekend
        is_weekend_closed = False
        
        if weekday_est == 5:  # Saturday - all day closed
            is_weekend_closed = True
            print(f"DEBUG - Saturday detected - market closed all day")
        elif weekday_est == 6:  # Sunday
            if time_decimal_est < 17.0:  # Before 5:00 PM EST on Sunday
                is_weekend_closed = True
                print(f"DEBUG - Sunday before 5:00 PM EST - market closed")
            else:
                print(f"DEBUG - Sunday after 5:00 PM EST - market OPEN (Asian session)")
        
        if is_weekend_closed:
            return {
                'name': 'Market Closed',
                'icon': '🔒',
                'description': 'Weekend - No Trading',
                'volatility': 'N/A',
                'color': '#6b7280',
                'current_time_est': now_est.strftime('%I:%M %p'),
                'current_time_local': now_eat.strftime('%I:%M %p'),
                'local_timezone': 'EAT (UTC+3)',
                'hour_est': hour_est,
                'minute_est': minute_est,
                'is_weekend': True,
                'trading_recommendation': '📅 Weekend - Markets are closed. Use this time to review your week and prepare for Monday.',
                'recommendation_color': 'gray',
                'active_pairs': [],
                'best_pairs': [],
                'kill_zones': [],
                'dead_zones': [],
                'correlation_warnings': ['Markets are closed on weekends. No trading activity.'],
                'news_drivers': ['Weekend - No major economic news. Check Monday\'s calendar.'],
                'current_kill_zone': None,
                'current_dead_zone': None,
                'is_dead_zone': False,
                'active_hours_local': 'Markets Closed',
                'next_session': 'Asian Session (Sunday 5:00 PM EST)',
                'time_until_next': 'Market reopens Sunday 5:00 PM EST'
            }
        
        print(f"DEBUG - Market is OPEN. Continuing with session detection...")
        ...
        sessions = [
            {
                ...
                'kill_zones': [...],  # Keep your existing data
                'dead_zones': [...],
                'active_pairs': [...],
                'best_pairs': ['EUR/USD', 'GBP/USD', 'XAU/USD'],
        ...
        for session in sessions:
            ...
            if start < end:
                if start <= time_decimal_est < end:
                    ...
                    break
            else:  # Handle sessions that DO span midnight (start > end)
                if time_decimal_est >= start or time_decimal_est < end:
                    matched_session = session
                    print(f"✓ MATCHED (overnight): {session['name']} at {time_decimal_est}")
                    break
        
        # If still no session found, default to a safe fallback
        if not matched_session:
            print(f"⚠ No session matched, using fallback")
            # Use London session as fallback (most active)
            for session in sessions:
                if session['name'] == 'London Session':
                    matched_session = session
                    break
            if not matched_session:
                matched_session = sessions[0] if sessions else None
        
        # If still no session, create a safe default
        if not matched_session:
            print(f"⚠ No sessions defined, creating safe default")
            matched_session = {
                'name': 'Unknown Session',
                'icon': '⚠️',
                'description': 'Unable to determine session',
                'volatility': 'N/A',
                'color': '#6b7280',
                'start': 0,
                'end': 24,
        ...
        import copy
        current_session = copy.deepcopy(matched_session)
        
        # ... rest of the function (kill zones, dead zones, etc.) ...
        ...
        current_session['is_weekend'] = False
        
        # Ensure all required fields exist
        if 'active_pairs' not in current_session:
            current_session['active_pairs'] = []
        if 'best_pairs' not in current_session:
            current_session['best_pairs'] = []
        if 'correlation_warnings' not in current_session:
            current_session['correlation_warnings'] = []
        if 'news_drivers' not in current_session:
            current_session['news_drivers'] = []
        if 'active_hours_local' not in current_session:
            current_session['active_hours_local'] = 'Unknown'
        if 'next_session' not in current_session:
            current_session['next_session'] = 'Unknown'
        if 'time_until_next' not in current_session:
            current_session['time_until_next'] = ''
        ...
        print(f"❌ ERROR in get_trading_session: {str(e)}")
        import traceback
        traceback.print_exc()
        ...
        return {
            'name': 'Unknown Session',
            'icon': '⚠️',
            'description': 'Session detection failed',
            'volatility': 'N/A',
            'color': '#6b7280',
            'is_weekend': False,
            'active_pairs': [],
            ...
            'current_dead_zone': None,
            'is_dead_zone': False,
            'trading_recommendation': 'Unable to determine session',
            'recommendation_color': 'gray',
            'active_hours_local': 'Unknown',
...
def is_market_open():
    """Check if any major market is currently open (including weekend check)"""
    est = pytz.timezone('US/Eastern')
    now_utc = timezone.now()
    now_est = now_utc.astimezone(est)
    hour_est = now_est.hour
    ...
    # Check if it's weekend (Saturday or Sunday in EST)
    weekday_est = now_est.weekday()  # 0 = Monday, 4 = Friday, 5 = Saturday, 6 = Sunday
    
    print(f"DEBUG is_market_open - Weekday: {weekday_est}, Hour: {hour_est}")
    
    # Forex market hours: Opens Sunday 5:00 PM EST, closes Friday 5:00 PM EST
    # Market is closed:
    # - Saturday (5) all day
    # - Sunday (6) BEFORE 5:00 PM EST
    if weekday_est == 5:  # Saturday - all day closed
        return {
            'is_open': False,
            'current_time_est': now_est.strftime('%I:%M %p'),
            ...
            'is_weekend': True,
            'next_open': 'Sunday 5:00 PM EST (Asian Open)'
        }
    elif weekday_est == 6:  # Sunday
        if time_decimal_est < 17.0:  # Before 5:00 PM EST
            return {
                'is_open': False,
                'current_time_est': now_est.strftime('%I:%M %p'),
                'market_status': 'CLOSED (Weekend)',
                'is_weekend': True,
                'next_open': 'Today at 5:00 PM EST'
            }
        else:
            # Sunday after 5:00 PM EST - market is OPEN (Asian session)
            is_open = True
            return {
                'is_open': True,
                'current_time_est': now_est.strftime('%I:%M %p'),
    ...
    # Check regular session hours (Monday - Friday)
    is_open = False
    
    # London Session (3:00 - 12:00 EST)
    if 3.0 <= time_decimal_est < 12.0:
        is_open = True
    
    # NY Session (8:00 - 17:00 EST)  
    if 8.0 <= time_decimal_est < 17.0:
        is_open = True
    
    # Asian Session (19:00 - 3:00 EST - overnight)
    if time_decimal_est >= 19.0 or time_decimal_est < 3.0:
        is_open = True
