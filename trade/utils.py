...
from .models import Trades, Pairs, Mood, FundedAccount
...
def train_model():
    """
    Train the ML model on GLOBAL training data (not user-specific).
    Uses real trade data from GlobalTrainingData table.
    """
    global MODEL
    from .models import GlobalTrainingData
    
    # Get all global training data with valid targets (0 or 1)
    trades_qs = GlobalTrainingData.objects.filter(target__in=[0, 1])
    trades_list = list(trades_qs.values(*FEATURES, "target"))

    if not trades_list:
        print("⚠ No global training data found. Please run: python manage.py copy_user_trades_to_global")
        print("  or add data via Django Admin first.")
        return None

    print(f"📊 Training model on {len(trades_list)} global trade records")
    
    # Count wins and losses
    win_count = sum(1 for t in trades_list if t["target"] == 1)
    ...
    if len(y.unique()) < 2:
        print("⚠ Not enough classes to train model. Need both wins AND losses.")
        print(f"   Found only: {y.unique()}")
        return None
    ...
    print(f"✅ Model trained successfully on {len(trades_list)} global samples.")
    print(f"   Training accuracy: {accuracy:.1%}")

    # Save the model for future fast loading
...
def get_model(force_retrain=False):
    ...
    try:
        if os.path.exists(MODEL_PATH):
            ...
            # Optional: Check if model is old and needs retraining
            return MODEL
...
def generate_unique_trade_id():
    """Generate a unique 4-digit trade ID with safety limit."""
    existing_ids = set(Trades.objects.values_list("trade_id", flat=True))
    
    # Check if we have too many IDs (avoid infinite loop)
    if len(existing_ids) >= 9000:  # 1000-9999 = 9000 possible IDs
        # Find first available ID
        for i in range(1000, 10000):
            if i not in existing_ids:
                return i
        # If all IDs are taken (extremely unlikely)
        return random.randint(100000, 999999)  # Fallback to 6-digit
    
    # Normal path - try random IDs
    attempts = 0
    while attempts < 100:
    ...
    # If we couldn't find a random ID, find sequentially
    for i in range(1000, 10000):
        if i not in existing_ids:
            return i
...
def get_trade_preview_data(trade_instance, rvs_value, rvs_grade, probability=None, rating=None):
    ...
    exclude_fields = {
        ...
        "reason", "holding_time_hrs", "holding_time", "narration", "user","draft_created_at","draft_updated_at","is_draft",
                ## tobe worked on letter
        "account", "risk_percent", "stop_loss_pips", "calculated_lot_size", "pip_value_used"
    }
...
def get_user_active_account(user):
    """Get the user's active funded account, or create one if none exists"""
    account = FundedAccount.objects.filter(user=user, is_active=True).first()
